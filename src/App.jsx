  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.04),transparent_30%),linear-gradient(to_bottom,rgba(15,23,42,0.2),rgba(2,6,23,0.95))]" />
      </div>

      <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        <header className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/30 backdrop-blur-xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-xs uppercase tracking-[0.4em] text-cyan-300/80">
                Blockchain Supply Chain Tracker
              </p>
              <h1 className="mt-2 max-w-3xl text-4xl font-semibold tracking-tight text-white sm:text-[3.4rem] sm:leading-[1.02]">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {pages.map((item) => (
                <button
                  ...
                  className={`rounded-[1.35rem] border px-4 py-3 text-left transition duration-200 ${
                    page === item.id
                      ? "border-cyan-300/80 bg-gradient-to-br from-cyan-400/20 to-emerald-400/10 text-white shadow-lg shadow-cyan-500/10"
                      : "border-white/10 bg-slate-900/60 text-slate-300 hover:border-white/20 hover:bg-slate-900/80"
                  }`}
                >
        ...
        <main className="mt-5 flex-1">
          <section className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-5 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
        ...
        <footer className="mt-6 flex flex-col gap-2 px-2 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
